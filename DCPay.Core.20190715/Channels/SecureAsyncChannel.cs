...
{
    public sealed class SecureAsyncChannel : AsyncChannel
    {
        public override event Action<IAsyncChannel, Packet> ReceivePacketEvent;

        public override event Action<IAsyncChannel, Packet> SendPacketEvent;
        ...
        private void SecureAsyncChannel_SendPacketEvent(IAsyncChannel channel, Packet pkg)
        {
            if (this.PublicKey != null)
            {
                //签名
                pkg.Signature = this.PrivateKey.Sign(pkg.GetSignBytes()).ToHex();
                Logger.Debug($"签名包[{pkg.PlatformId}({pkg.TimeStamp})][{pkg.Signature}]");
            }

            this.SendPacketEvent?.Invoke(channel, pkg);
        }

        private void SecureAsyncChannel_ReceivePacketEvent(IAsyncChannel channel, Packet pkg)
        {
            if (this.PublicKey != null)
            {
                if(string.IsNullOrWhiteSpace(pkg.Signature))
                {
                    Logger.Error($"空白签名，丢弃包[{pkg.PlatformId}][{pkg.TimeStamp}]");
                    return;
                }

                //验证签名
                if (this.PublicKey.Verify(pkg.GetSignBytes(), pkg.Signature.ToBinary()))
                {
                    Logger.Debug($"验证包的签名[{pkg.PlatformId}({pkg.TimeStamp})]通过");
                    this.ReceivePacketEvent?.Invoke(this, pkg);
                    return;
                }

                Logger.Warn($"验证包的签名[{pkg.PlatformId}({pkg.TimeStamp})]失败，丢弃。");
                return;
            }
