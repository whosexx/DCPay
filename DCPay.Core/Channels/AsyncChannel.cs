...
{
    public class AsyncChannel<T> : InitializedBase<Dictionary<string, string>>, IAsyncChannel<T> where T : class, IBitConverter, new()
    {
                    ...
                    try
                    {
                        ...
                        if(r.EndOfMessage)
                        {
                            ...
                            do
                            {
                                if (ms == null)
                                {
                                    data = new byte[r.Count];
                                    Array.Copy(buff, 0, data, 0, r.Count);
                                    break;
                                }
                            ...
                            {
                                var pkg = new T();
                                pkg.Unserialize(data);
                                if(pkg == null)
                                {
                                    Logger.Warn("反序列化有问题。");
                                    this.ReceiveUnknownDataEvent?.Invoke(this, data);
                                    continue;
                                }
                                
                                Logger.Debug($"通道[{this.Name}]收到业务包：" + pkg.ToString());
                                this.ReceivePacketEvent?.Invoke(this, pkg);
                            }
                            catch(Exception exf)
                            {
                                Logger.Error($"反序列化失败，可能是垃圾数据[{Encoding.UTF8.GetString(data)}]: [{exf}]");
                                this.ReceiveUnknownDataEvent?.Invoke(this, data);
                            }
        ...
        public virtual event Action<IAsyncChannel<T>, T> SendPacketEvent;
        ...
        public void Send(T pkg)
        {
            ...
            Logger.Debug($"通道[{this.Name}]发送业务包[{pkg.GetHashCode()}]：" + pkg.ToString());
            this.SendPacketEvent?.Invoke(this, pkg);
            lock (this.Client)
            {
                var bs = pkg.Serialize();
                int loop = 1;
                ...
                for (int i = 0; i < loop; i++)
                {
                    ...
                    if (!this.Client.SendAsync(new ArraySegment<byte>(bs, offset, count),
                        WebSocketMessageType.Text, (i == (loop - 1)), CancellationToken.None).Wait(this.SendTimeout))
                        throw new TimeoutException("发送数据包超时了。");
                }
            }
            Logger.Debug($"通道[{this.Name}]发送业务包成功[{pkg.GetHashCode()}]。");
        }
    }
}
